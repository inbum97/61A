...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    new_list=[paragraphs[i] for i in range(0,len(paragraphs)) if select(paragraphs[i])]
    if len(new_list)<=k:
        return ''
    if select(new_list[k])==True:
        return new_list[k]
    # END PROBLEM 1
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    num_dif=[diff_function(user_word, valid_words[i], limit) for i in range(len(valid_words))]

    if min(num_dif) > limit:
        return user_word
    min_diff = min(num_dif)
    
    return valid_words[num_dif.index(min_diff)]
    # END PROBLEM 5
...
def sphinx_swap(start, goal, limit):
    ...
    if start[0]!=goal[0]:
        if limit==0:
            return limit+1
        a=1+sphinx_swap(start[1:],goal[1:],limit-1)
        return a
    else:
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    result = []
    for action in times_per_player:
        temp = []
        for i in range(1, len(action)):
            diff = action[i] - action[i - 1]
            temp.append(diff)
        result.append(temp)
    answer = game(words, result)
    return answer
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for t in words:
        for i in players:
            fast_time = player_time[i][t]
            if not min_dic.get(t):
                min_dic[t] = fast_time
            if min_dic.get(t) > fast_time:
                min_dic[t]= fast_time
    ...
    for i in players:
        temp = [] 
        for t in words:
            if min_dic.get(t) == player_time[i][t]:
                temp.append(word_lst[t])
                min_dic.pop(t)
        final.append(temp)
    return final
