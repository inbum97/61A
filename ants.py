...
class Ant(Insect):
    ...
    blocks_path=True
    is_container=False
    is_doubled=False # ant hasn't be doubled yet
    # ADD CLASS ATTRIBUTES HERE
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        copy_beeLst = self.place.bees[:]
        if self.armor <= 0:
            for bee in copy_beeLst:
                bee.armor-=(amount+self.damage)
                if bee.armor<=0:
                    bee.death_callback()
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for index in list(self.place.bees):
            index.armor -= self.damage
            if index.armor <= 0:
                self.place.bees.remove(index)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.isReal:
            ...
            if p!=None:
                while p!=None:
                    if p.ant!=None:
                        if isinstance(p.ant,ContainerAnt) and p.ant.contained_ant != None and not p.ant.contained_ant.is_doubled:
                            p.ant.contained_ant.damage*=2
                            p.ant.contained_ant.is_doubled=True
                        if hasattr(p.ant, 'damage') and not p.ant.is_doubled:
                            p.ant.damage*=2
    ...
    def reduce_armor(self, amount):
        ...
        if  self.isReal and self.armor <= 0:
            self.place.remove_insect(self)
            self.death_callback()
            bees_win()
        elif    not self.isReal and self.armor<=0:
            self.place.remove_insect(self)
            self.death_callback()
        # END Problem 13

    def remove_from(self, place):
        if isinstance(place.ant,QueenAnt) and place.ant.isReal:
            return None
        if place.ant is self:
            place.ant = None
        elif place.ant is None:
            ...
            # container or other situation
            if isinstance(place.ant.contained_ant, QueenAnt) and place.ant.contained_ant.isReal:
                return None
            place.ant.remove_ant(self)
        Insect.remove_from(self, place)
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    f1 = bee.action
    f2 = effect(bee.action, bee)
    def apply(gamestate):
        nonlocal duration

        if effect == make_scare:
            if bee.first_scare == False:
                bee.scare_face = True
        if duration > 0:
            duration = duration - 1
            f2(gamestate)
            ...
            if effect == make_scare:
                bee.scare_face = False
                bee.first_scare = True
            f1(gamestate)
